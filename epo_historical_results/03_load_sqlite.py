#!/usr/bin/env python3
"""Load parsed EPO CSV files into SQLite (data/korem.db)."""
...
WORK = PROJECT / "work"
DATA_DIR = ROOT / "data"
CSV_DIR = WORK / "data" / "csv"  # existing parsed CSVs
DB_PATH = ROOT / "korem.db"
...
CREATE TABLE IF NOT EXISTS observations (
    ...
    value REAL,
    unit TEXT,
    UNIQUE (file_id, zone, section, participant, node, metric, hour),
    FOREIGN KEY (file_id) REFERENCES files(file_id)
);

CREATE INDEX IF NOT EXISTS idx_obs_delivery_date ON observations(delivery_date);
CREATE INDEX IF NOT EXISTS idx_obs_zone_section
    ON observations(zone, section, delivery_date);
CREATE INDEX IF NOT EXISTS idx_obs_participant
    ON observations(participant, delivery_date);
"""
...
def pdf_path_guess(csv_path: Path, rows: list[dict[str, str]]) -> str:
    if rows and rows[0].get("source_file"):
...
def load_csv(conn: sqlite3.Connection, csv_path: Path) -> int:
    ...
    trade_date = rows[0].get("trade_date") or "" if rows else ""
    delivery_date = rows[0].get("delivery_date") or "" if rows else ""

    conn.execute(
        """
        INSERT INTO files (file_id, trade_date, delivery_date, pdf_path, csv_path, parsed_at)
        VALUES (?, ?, ?, ?, ?, ?)
    ...
    obs = []
    for row in rows:
        # prefer file_id from CSV when present
    ...
    conn.executemany(
        ...
        INSERT INTO observations (
            file_id, trade_date, delivery_date, zone, section,
            participant, node, metric, hour, value, unit
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(file_id, zone, section, participant, node, metric, hour) DO UPDATE SET
            trade_date=excluded.trade_date,
            delivery_date=excluded.delivery_date,
...
def main() -> None:
    ...
    # recreate schema-compatible DB if old miner schema present
    ...
    try:
        ...
        if not args.force:
            existing = {r[0] for r in conn.execute("SELECT file_id FROM files").fetchall()}

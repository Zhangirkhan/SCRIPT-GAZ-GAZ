#!/usr/bin/env python3
"""Parse KOREM EPO result PDFs (folders 2023/2024/2025) into long-format CSV."""
...
PROJECT = ROOT.parent.parent  # Desktop/123
WORK = PROJECT / "work"  # existing year PDF folders
YEAR_DIRS = [WORK / y for y in ("2023", "2024", "2025")]
DATA_DIR = ROOT / "data"
...
def file_id_from_path(path: Path) -> int:
    # Stable id: "2023/file.pdf" (relative to work/), same as before the move
    try:
...
def csv_path_for(pdf_path: Path, delivery_date: str | None, file_id: int) -> Path:
    ...
    date_part = delivery_date or "unknown-date"
    return CSV_DIR / year / f"{date_part}_{file_id}.csv"
...
def main() -> None:
    ...
    parser.add_argument("--year", type=str, default="", help="Only one year folder: 2023/2024/2025")
    ...
    if not pdfs:
        print("No PDFs found in 2023/2024/2025")
        return
